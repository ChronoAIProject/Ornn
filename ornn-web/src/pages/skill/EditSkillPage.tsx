...
export function EditSkillPage() {
  ...
  const handleToggleVisibility = async () => {
    ...
    try {
      ...
      addToast({
        type: "success",
        message: skill.isPrivate
          ? t("editSkill.nowPublic", "Skill is now public")
          : t("editSkill.nowPrivate", "Skill is now private"),
      });
      refetch();
    } catch (err) {
  ...
  const handleUploadPackage = async () => {
    ...
    try {
      ...
      addToast({
        type: "success",
        message: t("editSkill.packageUpdated", "Skill package updated"),
      });
      setZipFile(null);
  ...
  const backTarget = `/skills/${encodeURIComponent(skill.name ?? id ?? "")}`;
  ...
  return (
    <PageTransition>
      <div className="h-full overflow-y-auto py-4">
        <nav className="mb-4">
          <BackLink
            label={t("editSkill.backToSkill", "Back to {{name}}", { name: skill.name })}
            to={backTarget}
          />
        </nav>
        ...
        <div className="mx-auto max-w-2xl space-y-6">
          ...
          <Card>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="font-body text-sm text-text-primary">
                  ...
                  <Badge color={skill.isPrivate ? "cyan" : "green"}>
                    {skill.isPrivate
                      ? t("editSkill.badgePrivate", "Private")
              ...
              >
                {skill.isPrivate
                  ? t("editSkill.makePublic", "Make Public")
                  : t("editSkill.makePrivate", "Make Private")}
              </Button>
            </div>
          </Card>
            ...
            <div className="space-y-4">
              ...
              {zipFile && (
                <div className="flex justify-end">
                  ...
                  >
                    {t("editSkill.uploadPackage", "Upload Package")}
                  </Button>
                </div>
              )}
            </div>
