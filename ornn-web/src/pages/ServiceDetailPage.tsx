...
export function ServiceDetailPage() {
  ...
  if (isLoading) {
    return (
      <PageTransition>
        <div className="py-4">
          <p className="font-body text-sm text-meta">Loading...</p>
        </div>
      </PageTransition>
    );
  }
  ...
  if (error || !service) {
    return (
      <PageTransition>
        <div className="py-4">
          <p className="font-body text-sm text-danger">Failed to load service: {error ?? "Not found"}</p>
    ...
    <PageTransition>
      <div className="py-4">
        <button
          ...
          className="flex items-center gap-2 font-body text-sm text-meta hover:text-accent transition-colors mb-4 cursor-pointer"
        >
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
              <h1 className="font-heading text-2xl tracking-wider text-strong">{service.name}</h1>
          ...
          {service.description && (
            <p className="font-body text-sm text-meta">{service.description}</p>
          )}
        </div>

        <div className="space-y-4">
          <Card>
            <h3 className="font-heading text-[10px] font-700 tracking-widest uppercase text-meta mb-2">NyxID Proxy URL</h3>
            <p className="font-mono text-xs text-accent break-all">{service.proxyUrl}</p>
          </Card>

          <Card>
            <h3 className="font-heading text-[10px] font-700 tracking-widest uppercase text-meta mb-2">OpenAPI Spec URL</h3>
            {service.openapiProxyUrl ? (
              ...
              <p className="font-body text-xs text-meta italic">No OpenAPI spec configured</p>
            )}
          </Card>

          <Card>
            <h3 className="font-heading text-[10px] font-700 tracking-widest uppercase text-meta mb-2">Source Code</h3>
            {service.repositoryUrl ? (
              ...
              <p className="font-body text-xs text-meta italic">Not configured</p>
            )}
          </Card>

          <Card>
            <h3 className="font-heading text-[10px] font-700 tracking-widest uppercase text-meta mb-2">Homepage</h3>
            {service.homepageUrl ? (
