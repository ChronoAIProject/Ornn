    ...
    <Modal isOpen={isOpen} title="Generate Skill" onClose={handleClose}>
      {step === "select" && (
        <div className="space-y-4">
          <p className="font-body text-sm text-text-muted">
            Select references to include in skill generation for <strong className="text-text-primary">{serviceName}</strong>.
          </p>

          {/* Reference list */}
          <div className="space-y-2">
            {references.map((ref) => (
              <div key={ref.id} className="flex items-center gap-3 px-3 py-2 rounded-lg border border-neon-cyan/10 bg-bg-elevated/30">
                <input
                  type="checkbox"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs text-text-primary truncate">{ref.label}</span>
                  </div>
                </div>
                {!ref.auto && (
                  <button onClick={() => removeRef(ref.id)} className="text-text-muted hover:text-neon-red text-xs cursor-pointer">
                    Remove
                  </button>
                )}
              </div>
          ...
          <div className="space-y-2">
            ...
            {urlError && <p className="font-body text-xs text-neon-red">{urlError}</p>}
          </div>

          {/* Proceed */}
          <div className="flex justify-end gap-3 pt-2 border-t border-neon-cyan/10">
      ...
      {step === "generating" && (
        <div className="flex flex-col items-center justify-center py-8 space-y-4">
          ...
          <p className="font-body text-sm text-text-primary">{currentStepMsg}</p>
          <p className="font-body text-xs text-text-muted">This may take 30-60 seconds...</p>
        </div>
      )}
      ...
      {step === "done" && (
        <div className="flex flex-col items-center justify-center py-8 space-y-4">
          ...
          <p className="font-body text-sm text-text-primary">Skill generated successfully!</p>
          <p className="font-body text-xs text-text-muted">Redirecting to System Skills...</p>
        </div>
      )}
      ...
      {step === "error" && (
        <div className="space-y-4 py-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-neon-red/10 flex items-center justify-center shrink-0">
              <svg className="w-5 h-5 text-neon-red" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </div>
            <div>
              <p className="font-body text-sm text-text-primary">Generation failed</p>
              <p className="font-body text-xs text-neon-red">{errorMsg}</p>
            </div>
          </div>
