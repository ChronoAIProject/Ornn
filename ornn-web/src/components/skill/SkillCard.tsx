...
export function SkillCard({
  ...
  return (
    ...
    >
      {/* Title — own row, full card width so long names get the room
          they need before truncating. Wrap up to two lines, then ellipsis. */}
      <h3 className="mb-2 font-display text-lg font-semibold text-accent break-words line-clamp-2">
        {skill.name}
      </h3>

      {/* Permission / system / GitHub-source badges — separate row so the
          title isn't fighting them for horizontal space. */}
      <div className="mb-3 flex flex-wrap items-center gap-1.5">
        {skill.hasGithubSource && (
          <span
            className="inline-flex h-5 w-5 items-center justify-center text-meta"
            aria-label={t("skillComponents.card.linkedToGithub")}
            title={t("skillComponents.card.linkedToGithub")}
          >
