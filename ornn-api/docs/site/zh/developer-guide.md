...
Ornn 提供 REST API，AI Agent 可以用来发现和执行技能。Agent 通过 NyxID 认证（JWT 或 API Key）连接到 ornn-api 的 API 端点。
...
sequenceDiagram
    ...
    ornn-api-->>Agent: SSE text-delta + 完成
```
...
Ornn 提供三个**核心技能**，教 AI Agent 如何与平台交互。它们位于 chrono-ornn 仓库的 [`.ornn-apis/`](https://github.com/aevatarAI/chrono-ornn/tree/main/.ornn-apis) 目录中。
