...
export interface SkillsetPluginInput {
   ...
   * plugin version or the bundled files — only the README note — since the
...
export function buildSkillsetPlugin(
  ...
  // The update signal CC actually compares: the skillset's auto-managed
  // revision (#1162). The service bumps the revision's minor whenever a
  // member's resolved version moves (before this builder runs), so a moving
  // `@latest` member's new version reaches installed plugins via the bumped
  // revision — no `+sk` fingerprint, plugin.json and marketplace agree.
  ...
  return {
    files,
    // plugin.json and the catalogue entry now carry the SAME revision (#1162) —
    // no `+sk` fingerprint to diverge — so the shared marketplace.json stays in
    // lockstep with plugin.json across the full-reconcile and skill-incremental
    // code paths.
