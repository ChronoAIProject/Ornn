...
function generateFromSourceOperation(): Record<string, unknown> {
  return {
    ...
    description:
      ...
      "There is no `mode` here — the output is always the equivalent of `simple` (a `plain` skill with empty `scripts`, `references` and `assets`); a `mode` key in the body is ignored, not rejected. " +
      GENERATION_STREAM_CONTRACT +
      " " +
      GENERATION_COST_CONTRACT +
...
function generateFromOpenApiOperation(): Record<string, unknown> {
  return {
    ...
    description:
      ...
      "There is no `mode` here either — the output is always the equivalent of `simple` (a `plain` skill with empty `scripts`, `references` and `assets`); a `mode` key in the body is ignored, not rejected. " +
      GENERATION_STREAM_CONTRACT +
      " " +
      GENERATION_COST_CONTRACT +
