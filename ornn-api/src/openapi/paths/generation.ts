...
const generateMultipartBody: JsonSchema = {
  ...
  properties: {
    ...
    mode: {
      ...modeProperty,
      description:
        "Same semantics as the JSON body's `mode`. Sent as a plain form field — a file part or a repeated field under this name fails with 400 `invalid_mode`. Note that in `simple` mode an attached `package` is still read as context in full (including its `scripts/`), but the answer is still required to be `SKILL.md`-only.",
    },
    package: {
      type: "string",
      format: "binary",
  ...
  return {
    ...
    responses: {
      ...
      ...problemResponses(
        {
          400:
            "Rejected before the stream opened. Codes: `invalid_content_type` (neither JSON nor multipart), `invalid_body` (unparseable JSON, or valid JSON that is an array or a scalar rather than an object), `missing_prompt` (no usable `prompt` — an empty body lands here, since it is read as `{}`), `prompt_too_long` / `content_too_long` (over 32 000 characters), `invalid_mode` (`mode` is not `simple` or `advanced`), `MODEL_NOT_FOUND` / `MODEL_NOT_ENABLED` (the `modelId` you asked for is unknown, or not enabled for the `skillGen` surface).",
        },
        401,
...
function generateFromSourceOperation(): Record<string, unknown> {
  return {
    ...
    description:
      ...
      "There is no `mode` here. The prompt asks for a `plain`, file-free skill (the shape `simple` mode produces) but, unlike `simple`, this path does not enforce that server-side — validate `raw` before publishing. A `mode` key in the body is ignored, not rejected. " +
      GENERATION_STREAM_CONTRACT +
      " " +
      GENERATION_COST_CONTRACT +
...
function generateFromOpenApiOperation(): Record<string, unknown> {
  return {
    ...
    description:
      ...
      "There is no `mode` here either. The prompt asks for a `plain`, file-free skill (the shape `simple` mode produces) but the server does not enforce that on this path — validate `raw` before publishing. A `mode` key in the body is ignored, not rejected. " +
      GENERATION_STREAM_CONTRACT +
      " " +
      GENERATION_COST_CONTRACT +
